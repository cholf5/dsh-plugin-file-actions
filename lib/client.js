window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function dispatchSelection(id, cardProps, deps) {
      ...
      var fail = function (result, app) {
        var code = result.data !== null && result.data !== undefined && result.data.code !== undefined
          ? result.data.code : '';
        if (code === 'no-command') deps.setError(t('error.noCommand'));
        else if (code === 'unavailable-app') deps.setError(t('error.unavailableApp'));
        else if (code === 'unavailable-terminal') deps.setError(t('error.unavailableTerminal'));
        else if (code === 'launch-failed') deps.setError(t('error.launchFailed', { app: app }));
        else deps.setError(t('error.generic'));
      };
      if (id === 'fa:copy-rel') {
        deps.close();
    ...
    async function apply(ctx) {
       ...
       * the recorder cell below. The whole object is replaced on every open,
       * and the render closures always read the variable, never a stale copy.
       */
      var contextState = { open: false, x: 0, y: 0, path: null, cwd: null, error: null };
      ...
      /**
       * The right-click context menu over one message file link, anchored at
       * the cursor through Menu's getAnchorRect (portal mode; the viewport
       * clamp keeps the panel on screen). Items are the card menu's, built
       * from a `{ file: { path }, cwd }` stand-in.
       */
      function LinkMenu(props) {
        var menu = props.menu;
        var open = menu.open === true && menu.path !== null && menu.path !== undefined;
        var target = { file: { path: menu.path }, cwd: menu.cwd };
        return e(Menu, {
          open: open,
          autoFocus: true,
          portal: true,
          align: 'start',
          side: 'bottom',
          items: open ? buildItems(target, props.state, props.t, { error: menu.error }) : [],
          onSelect: function (id) {
            dispatchSelection(id, target, props);
          },
      ...
      /**
       * The message file links: the official markdown renders every file
       * mention and every markdown file link as a button carrying the path in
       * its `title` (shared hashed fileMention class; the input area's
       * reference chips share the class but mark themselves with
       * `data-ref-chip`, so they are excluded). Pure delegation — nothing is
       * inserted among React-managed children.
       */
      var LINK_SELECTOR = 'button[class*="fileMention"][title]:not([data-ref-chip])';

      function onContextMenu(event) {
        var target = event.target;
        if (target === null || target === undefined || typeof target.closest !== 'function') return;
        var button = target.closest(LINK_SELECTOR);
        if (button === null) return;
        var path = button.getAttribute('title');
