window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     * relativizeToCwd: newer hosts present presented-file paths absolutely,
     * so the copy-relative entry must relativize instead of copying as-is. */
    function relativizeToCwd(text, cwd) {
      if (cwd === undefined || cwd === '') return text;
      var root = cwd.replace(/[/\\]+$/, '');
      if (text.indexOf(root + '/') === 0 || text.indexOf(root + '\\') === 0) return text.slice(root.length + 1);
      return text;
    }
    ...
    function buildItems(cardProps, state, t, actions) {
      ...
      if (state.info !== null && state.info !== undefined) {
        ...
        var editors = pick(EDITOR_IDS);
        var terminals = pick(TERMINAL_IDS);
    ...
    function CardMenu(props) {
      ...
      var select = function (id) {
        ...
        var fail = function (result, app) {
          ...
          else if (code === 'unavailable-app') setError(t('error.unavailableApp'));
          else if (code === 'unavailable-terminal') setError(t('error.unavailableTerminal'));
