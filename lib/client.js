window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     * the menu. `mode` trims the surface: the card slot cell rides beside the
     * official open-in-app control (which owns default-app open, the OS
     * association list, and reveal), so it drops the duplicated file-manager
     * and editor sections and contributes only terminals + copies; the
     * message-link context menu keeps every section — there is no official
     * alternative on a message link.
     */
    function buildItems(cardProps, state, t, actions, mode) {
    ...
    async function apply(ctx) {
      ...
      /**
       * One card's dropdown menu, rendered as an official
       * deliverables.file.actions slot cell (the 0.1.7 contributed-actions
       * architecture). The cell registers under its own id with order 10, so
       * it renders BESIDE the shipped open-in-app cell instead of replacing
       * it: the official control keeps what it is best at (default-app open,
       * the per-file OS association list, reveal), and this plugin
       * contributes what it does not have — run in terminal, open the
       * containing directory in terminal, copy paths. buildItems' 'slot'
       * mode drops the sections that would duplicate the official control;
       * the message-link context menu keeps every section.
       *
      ...
      // The official deliverables card seat: registering under its own id
      // (a fresh id is added beside the shipped entries) with order 10 keeps
      // the plugin cell after the official open-in-app control — coexistence,
      // not replacement (see CardMenu). ctx.slots.inject runs the callback per
      // declaration lifetime and unwinds the registration when this plugin's
      // fiber unloads — the same lifecycle the session-header cwd recorder
      // below rides.
      ...
      styleTag.textContent = [
        // The card menu trigger: the official 0.1.7 compact control's
        // geometry (24px hairline pill, secondary label, hover fill) — the
        // official classes are hashed, so the plugin carries its own.
        '[data-fa-trigger] {',
