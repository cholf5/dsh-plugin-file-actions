window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    async function apply(ctx) {
       ...
       * architecture: the card no longer ships its own chevron to take over).
       * The cell registers under the shipped id 'open-in-app' at a lower
       * priority than the official registration (default 0) — the slot ledger
       * elects the lowest-priority live entry per id cell, so the official
       * open-in-app control is shadowed out. That is the supported replacement
       * for the pre-0.1.7 hide-the-chevron takeover.
       *
      ...
      function CardMenu(props) {
        ...
        var items = path === null
          ? []
          : buildItems({ file: { path: path }, cwd: cwd }, state, props.t, { error: error });
      ...
      // The official deliverables card seat: registering under the shipped id
      // with a lower priority shadows the official open-in-app cell (see
      // CardMenu). ctx.slots.inject runs the callback per declaration lifetime
      // and unwinds the registration when this plugin's fiber unloads — the
      // same lifecycle the session-header cwd recorder below rides.
