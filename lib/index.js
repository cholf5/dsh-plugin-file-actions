 ...
 * built from the configured extension map plus the file path; on Windows the
 * command line reaches the terminal's shell through an environment variable,
 * never through a re-quoted argument. Launches spawn detached through the
 * official launcher with a credential-scrubbed environment.
 */
...
export const Config = z.object({
  ...
  /** Permit running files whose extension is unmapped but that carry an execute bit (extension-derived on Windows). */
 ...
 * @returns null when no run spelling applies (unmapped extension, no execute bit).
 */
function runInvocationFor(file, config, fileStat, platform) {
  ...
  if (config.allowExecutableBit && (fileStat.mode & 0o111) !== 0) {
 ...
 *   this platform (the route answers 400).
 */
async function runInTerminal(app, file, invocation, context) {
  ...
  const launcher = resolved.launch
  ...
  if (platform === 'win32') {
    ...
    if (app === 'gitbash') {
      ...
      // Windows layout ships mintty and bash beside it. CHERE_INVOKING keeps
      // the login shell from cd-ing home after our cd.
      const gitRoot = path.dirname(executable)
      await launch(path.join(gitRoot, 'usr', 'bin', 'mintty.exe'), [
        '-e', path.join(gitRoot, 'usr', 'bin', 'bash.exe'), '-c',
        `cd ${shellQuote(directory)} && ${posixCommand}; exec bash -l -i`,
 ...
 * @param seam - test seams: resolver/catalog/launch/runCommand/platform/env/home/ssh/resolveExecutable.
 */
export async function apply(ctx, config, seam = {}) {
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      else sendJson(res, 400, { code: 'bad-request', message: `unsupported terminal: ${parsed.app}` })
    },
